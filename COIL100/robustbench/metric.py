...
def get_cluster_sols(x, cluster_obj=None, ClusterClass=None, n_clusters=None, init_args={}):
    ...
    return cluster_assignments, cluster_center
...
def clustering_metric(y_true, y_pred, n_clusters, decimals=4):
    ...
    return acc, nmi, ari, class_acc, p, fscore, y_pred
...
def clustering_by_representation(X_rep, y,n_clusters):
    """Get scores of clustering by representation"""
    # y = y.cpu().numpy()

    # X_rep = X_rep.cpu().numpy()

    kmeans_assignments, cluster_center = get_cluster_sols(X_rep, ClusterClass=KMeans, n_clusters=n_clusters,
                                             init_args={'n_init': 10, 'random_state': 42})


    # print('kmeans_assignments',kmeans_assignments)
    # np.save("64dim-source_result.npy", kmeans_assignments)
    # np.save("64dim-cluster_center.npy", cluster_center)
    # source_result = np.load("source_result.npy")
    # print('source_result',source_result)

    if np.min(y) == 1:
        y = y - 1
    acc, nmi, ari, class_acc, p, fscore, y_pred = clustering_metric(y, kmeans_assignments,n_clusters)
    return acc, nmi, ari, class_acc, p, fscore, kmeans_assignments,cluster_center
